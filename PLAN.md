...
- **CORS coordination — non-trivial.** `X-Request-Id` is **not** a CORS-safelisted request header, so adding it unconditionally turns every API call into a preflighted request, and the cross-origin response value is unreadable to the client unless the server explicitly exposes it. Logos today allows only `Accept`, `Authorization`, `Content-Type` (`Access-Control-Allow-Headers`) and exposes only `Link` (`Access-Control-Expose-Headers`), so a naive rollout fails preflight from `localhost:5173 → localhost:8000` in dev and silently drops the response id everywhere even when the server emits it. Two coordinated changes are required, in order:
  1. **Logos PR** — extend the CORS allow-list and expose-list with `X-Request-Id`. Important: only `CORS_ALLOWED_ORIGINS` is currently env-var driven (per §4.5); the `AllowedHeaders` and `ExposedHeaders` slices are **hard-coded in `internal/router/router.go`** today (`Accept`, `Authorization`, `Content-Type` for allow; `Link` for expose), so there is no env knob to flip. Two viable approaches for the Logos PR, pick one:
     - **(a) Match the origins pattern, add env-var config knobs** — introduce `CORS_ALLOWED_HEADERS` and `CORS_EXPOSE_HEADERS` env vars alongside `CORS_ALLOWED_ORIGINS`, default them to the current hard-coded slices plus `X-Request-Id`, and read them at router init. More code, but the CORS surface ends up consistently configurable and the cluster-side override path matches §4.5 for free. Preferred long-term.
     - **(b) Directly extend the hard-coded slices** — append `"X-Request-Id"` to both literals in `internal/router/router.go`. One line per list, zero new config surface, no env-var plumbing. Cheaper, but the next CORS tweak will hit the same coordination cost again.
     Either way, the in-cluster behavior is unchanged for the same-origin production path; the change only matters for cross-origin dev (`localhost:5173 → localhost:8000`).
